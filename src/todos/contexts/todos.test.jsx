...
import { TodosContext, TodosProvider, addTodo, initialState, reducer } from './todos'
...
describe('TodosContext reducer', () => {
  ...
  it('addTodo', () => {
    const newTodo = { id: '1', text: 'foo', isCompleted: false }
    const newState = reducer(initialState, { type: 'addTodo', payload: newTodo })
    expect(newState.todos[0]).toEqual(newTodo)
  })

  it('toggleAll', () => {
    const oldState = { ...initialState, todos: [{ id: '1', text: 'foo', isCompleted: false }] }
    const newState = reducer(oldState, { type: 'toggleAll', payload: true })
    expect(newState.todos[0].isCompleted).toEqual(true)
  })

  it('updateTodo', () => {
    const oldState = { ...initialState, todos: [{ id: '1', text: 'foo', isCompleted: false }] }
    const newState = reducer(oldState, { type: 'updateTodo', payload: { id: '1', text: 'bar' } })
    expect(newState.todos[0].text).toEqual('bar')
  })

  it('removeTodo', () => {
    const oldState = { ...initialState, todos: [{ id: '1', text: 'foo', isCompleted: false }] }
    const newState = reducer(oldState, { type: 'removeTodo', payload: '1' })
    expect(newState.todos).toHaveLength(0)
  })

  it('changeFilter', () => {
    const newState = reducer(initialState, { type: 'changeFilter', payload: 'active' })
...
describe('api', () => {
  ...
  it('addTodo', async () => {
    ...
    expect(mockDispatch).toHaveBeenCalledWith({ type: 'addTodo', payload: mockResponse.data })
  })
})
