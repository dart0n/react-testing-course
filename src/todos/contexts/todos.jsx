...
export const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'toggleAll': {
      const updatedTodos = state.todos.map((todo) => ({ ...todo, isCompleted: action.payload }))
...
export const getTodos = async (dispatch) => {
  ...
  dispatch({ type: 'getTodos', payload: response.data })
}

export const addTodo = async (dispatch, todoToCreate) => {
  const response = await axios.post('http://localhost:3004/todos', todoToCreate)
  dispatch({ type: 'addTodo', payload: response.data })
}

export const updateTodo = async (dispatch, todoId, fieldsToUpdate) => {
  const response = await axios.put(`http://localhost:3004/todos/${todoId}`, fieldsToUpdate)
...
export const toggleAll = async (dispatch, isCompleted, todos) => {
  const promises = todos.map((todo) => {
    return axios.put(`http://localhost:3004/todos/${todo.id}`, { text: todo.text, isCompleted })
  })
  await Promise.all(promises)
  dispatch({ type: 'toggleAll', payload: isCompleted })
}
    ...
    <TodosContext.Provider
      value={[
        ...
        dispatch,
        {
          ...
          updateTodo: (todoId, fieldsToUpdate) => updateTodo(dispatch, todoId, fieldsToUpdate),
          removeTodo: (todoId) => removeTodo(dispatch, todoId),
          changeFilter: (filter) => changeFilter(dispatch, filter),
          toggleAll: (isCompleted) => toggleAll(dispatch, isCompleted, state.todos)
        }
      ]}
    >
      {children}
