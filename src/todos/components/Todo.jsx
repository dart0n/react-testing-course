...
const Todo = ({ todo, isEditing, setEditingId }) => {
  ...
  return (
    <li className={`${editingClass} ${completedClass}`} data-testid="todo" data-cy="todo">
      <div className="view">
        <input
          className="toggle"
          data-testid="toggle"
          ...
          onChange={toggleTodo}
        />
        <label onDoubleClick={setTodoInEditingMode} data-testid="label" data-cy="todoLabel">
          {todo.text}
        </label>
        <button className="destroy" data-cy="destroy" onClick={() => removeTodo(todo.id)} data-testid="destroy" />
      </div>
      {isEditing && (
        <input
          className="edit"
